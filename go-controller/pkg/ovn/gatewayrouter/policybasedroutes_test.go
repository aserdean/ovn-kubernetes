...
func TestAddCrossNodeHostIPPolicy(t *testing.T) {
	...
	}{
		{
			...
			expectedDB: []libovsdbtest.TestData{
				...
				&nbdb.LogicalRouterPolicy{
					...
					Match:    generateLocalDeliveryCrossNodeMatch(transitPort, v4Prefix, nodeHostIPv4Str, nodeName),
					Action:   nbdb.LogicalRouterPolicyActionReroute,
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			...
			err = mgr.AddCrossNodeHostIPPolicy(transitPort, nodeName, tt.mgntIP, tt.hostInfCIDR, tt.otherHostAddrs)
			if tt.expectErr {
				if err == nil {
