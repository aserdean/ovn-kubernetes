...
func makeNodeRouterTargetIPs(node *nodeInfo, c *lbConfig, hostMasqueradeIPV4, hostMasqueradeIPV6 string) (targetIPsV4, targetIPsV6 []string, v4Changed, v6Changed bool) {
	...
	if config.OvnKubeNode.Mode == types.NodeModeDPU {
		...
		targetIPsV4, v4Updated = util.UpdateIPsSlice(targetIPsV4, node.dpuHostAddressesStr(), []string{hostMasqueradeIPV4})
		targetIPsV6, v6Updated = util.UpdateIPsSlice(targetIPsV6, node.dpuHostAddressesStr(), []string{hostMasqueradeIPV6})
	} else {
					...
					if vip == placeholderNodeIPs {
						if !node.nodePortDisabled {
							if config.OvnKubeNode.Mode == types.NodeModeDPUHost {
								vips = append(vips, node.dpuHostAddressesStr()...)
							} else {
