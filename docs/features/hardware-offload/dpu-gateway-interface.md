...
In DPU deployments, the host typically has access to Virtual Functions (VFs) for management purposes, while the Physical Functions (PFs) are used for external connectivity. The "from-pci-address" feature allows OVN-Kubernetes to automatically discover and configure the appropriate PF interface as the gateway interface based on the VF used for the management port.

## How It Works

When configured with `--gateway-interface=from-pci-address`, OVN-Kubernetes performs the following steps:
...
--gateway-interface=from-pci-address
```
...
With `--gateway-interface=from-pci-address`, OVN-Kubernetes will:
...
- Gateway interface must be set to `from-pci-address`
...
The "from-pci-address" feature can be used in conjunction with gateway accelerated interface configuration. When both are configured, the automatically resolved gateway interface will be used for hardware acceleration.
