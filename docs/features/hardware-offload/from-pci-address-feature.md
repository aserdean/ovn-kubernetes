...
The "from-pci-address" gateway interface feature is a new capability in OVN-Kubernetes that enables automatic gateway interface resolution in DPU (Data Processing Unit) host mode deployments. This feature automatically discovers and configures the appropriate Physical Function (PF) interface as the gateway interface based on the Virtual Function (VF) used for the management port.
...
The "from-pci-address" feature automates the gateway interface discovery process by:
...
1. **New Constant**: Added `FromPCIAddress = "from-pci-address"` constant in `go-controller/pkg/types/const.go`
...
- [Gateway Accelerated Interface Configuration](../design/gatway-accelerated-interface-configuration.md)
